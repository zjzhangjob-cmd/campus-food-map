...
def add_caohejing_data():
    db = SessionLocal()
    try:
        # 漕河泾坐标：北纬31.17，东经121.39
        CAOHEJING_CENTER = (31.17, 121.39)
        # 漕河泾主要区域
        CAOHEJING_AREAS = [
            ("漕河泾开发区", CAOHEJING_CENTER),
            ("漕河泾印象城", (31.165, 121.395)),
            ("漕河泾地铁站", (31.168, 121.388)),
            ("田林路", (31.175, 121.398)),
            ("古美路", (31.185, 121.405)),
            ("虹梅路", (31.165, 121.408)),
            ("桂林路", (31.178, 121.382)),
        ...
        STREETS = ["田林路", "古美路", "虹梅路", "桂林路", "宜山路", "漕溪路", "柳州路", "苍梧路", "虹漕路", "桂平路"]
        DESC_WORDS = ["味道正宗", "性价比高", "环境舒适", "服务周到", "人气火爆", "适合办公", "商务宴请", "团队聚餐"]
        ...
        existing = db.query(Restaurant).filter(
            Restaurant.latitude.between(31.14, 31.21),
            Restaurant.longitude.between(121.35, 121.45)
        ).count()

        to_add = target_count - existing
        print("漕河泾现有餐厅:", existing)
        print("需要新增:", to_add)
        ...
        for i in range(to_add):
            ...
            area_name, (base_lat, base_lng) = random.choice(CAOHEJING_AREAS)
            ...
            suffixes = ["", "（漕河泾店）", "（田林路店）", "（虹梅路店）", "（古美路店）", "（旗舰店）"]
            ...
            address = "徐汇区" + street + str(street_num) + "号"
            ...
            desc = cuisine + "餐厅，位于漕河泾开发区，" + random.choice(DESC_WORDS) + "。"
            ...
            r = Restaurant(
                ...
                price_min=price_min,
                price_max=price_max,
                distance_min=random.randint(20, 300),
                emoji=emoji,
                tags=",".join(tags[:8]),
            ...
            if (i + 1) % batch_size == 0:
                db.flush()
                print("已生成", i + 1, "/", to_add, "家餐厅")

        db.flush()
        print("已生成", len(restaurants), "家漕河泾餐厅")
        ...
        for rest in restaurants:
            if random.random() < 0.5:
                ...
                deal_types = [
                    ...
                    ("工作日午餐", "主食+汤+小菜", max(25, int(avg_p * 0.7)), max(45, int(avg_p * 0.95))),
                ]
        ...
        db.commit()
        final_count = db.query(Restaurant).filter(
            Restaurant.latitude.between(31.14, 31.21),
            Restaurant.longitude.between(121.35, 121.45)
        ).count()
        print("✅ 完成！漕河泾餐厅总数:", final_count)

        menu_count = db.query(MenuItem).filter(
            MenuItem.restaurant.has(
                Restaurant.latitude.between(31.14, 31.21),
                Restaurant.longitude.between(121.35, 121.45)
            )
        ).count()
        review_count = db.query(Review).filter(
            Review.restaurant.has(
                Restaurant.latitude.between(31.14, 31.21),
                Restaurant.longitude.between(121.35, 121.45)
            )
        ).count()
